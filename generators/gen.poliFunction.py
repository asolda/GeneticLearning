...
testfilename = "poliFunction.testset"
...
def evaluate(individual):
    ...
    for x in individual:
        term1 += (x - 5)**2
        term2 += ((x + 4)**2)/8 + ((x+1)**2)/27 + 15
        term3 += (1/((x**2) + 1)) - 1.1 * np.exp(-(x**2))
        term4 += x
    return term1, term2, term3, term4, sum(individual[1::2]) - sum(individual[::2])
